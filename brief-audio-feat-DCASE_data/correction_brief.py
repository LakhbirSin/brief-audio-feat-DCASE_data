...
solverM = ['linear','rbf','poly','sigmoid']
parameters = dict(kernel=solverM)
kl = svm.SVC(C=300, class_weight=None)
model = GridSearchCV(kl, parameters, cv=5)

#-------------------------------------------------------------------------------------------------------------------------------------------------

# solverM = ['liblinear', 'newton-cg', 'lbfgs', 'sag', 'saga','newton-cholesky']
# parameters = dict(solver=solverM)
# lr = LogisticRegression(class_weight=None,multi_class="auto", n_jobs=-1, C=100)
# model = GridSearchCV(lr, parameters, cv=2)
...
# Matrix confusion
#ConfusionMatrixDisplay.from_predictions(testLabelsStd, y_pred) 
#print("Score :" , model.score(testFeatures_scaled, testLabelsStd))

model.cv_results_['mean_test_score']
scores = model.cv_results_['mean_test_score']
for score, solver, in zip(scores, solverM):
    print(f"{solver}: {score:.3f}")



sns.barplot(x=solverM, y=scores).set_title("Accuracy with Scaled Features", fontsize="20")
